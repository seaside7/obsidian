
With over a decade of experience spanning product management, engineering leadership, and data-driven platforms across startups and large organizations, I bring a strong track record of delivering complex systems and scaling products in challenging environments.



Most recently, I served as VP of Product and Technology at Manuva, where I led cross-functional product, engineering, design, and data teams to build and scale an integrated manufacturing and supply platform serving both B2B and B2R customers. Prior to that, I held senior product leadership roles at Finantier, RedDoorz, and Payfazz, where I owned product strategy, roadmap execution, and delivery for high-impact platforms in fintech and consumer technology.

A Product Manager is accountable for product value and direction, while a Scrum Master is accountable for how effectively teams deliver that value.

## 7️⃣ Why companies need BOTH

- Without PM → teams build the wrong thing
    
- Without SM → teams build the right thing poorly or too slowly

## 🎯 Core idea you want to communicate

You are **not “moving away” from Product**.  
You are **focusing on delivery excellence**, where you have repeatedly seen the biggest impact.

## ✅ Full interview answer (60–90 seconds)

> Over the years, I’ve held product leadership roles where I owned strategy, roadmap, and outcomes. What I consistently observed is that even with a strong product vision, execution often becomes the limiting factor—especially in complex, fast-moving environments with multiple stakeholders.
> 
> I found that I was spending a significant amount of time helping teams improve delivery flow, resolve blockers, align expectations, and make trade-offs explicit. That part of the work is where I saw the most immediate impact.
> 
> Moving into a Scrum Master role is a deliberate decision to focus on delivery excellence—coaching teams, improving predictability, and enabling engineers and product owners to succeed—rather than dividing attention across strategy and execution. Given my background, I bring strong product context into the role, which helps me be an effective partner rather than just a process facilitator.


## ✅ Short version (30 seconds)

> I’ve led product strategy before, but I’ve seen that execution is often the real bottleneck. The Scrum Master role allows me to focus on improving delivery flow, removing impediments, and coaching teams—areas where I’ve consistently had the most impact.


I’m comfortable operating at different levels as long as expectations are clear. My goal is to add value by enabling teams to deliver effectively, not to own authority.


In my role at Manuva, I initially received a salary alongside equity. However, as market conditions became more challenging and fundraising slowed, the company shifted its focus toward managing runway and asset restructuring. During this period, compensation was adjusted, and the company is currently in the process of selling its assets.

Given this situation, I’m now looking to return to a stable salaried role. At the same time, this transition gave me clarity on where I create the most impact—particularly in delivery excellence, helping teams execute effectively, and navigating complexity. That’s why I’m intentionally exploring roles where I can focus on enabling strong execution and predictable outcomes.


## 🧭 Day-to-Day Activities of a Scrum Master

Think of the role as **delivery flow + people + system health**.

## 🌅 Morning (Start of day)

### 1️⃣ Review delivery signals (15–30 min)

You typically check:

- Sprint progress / board flow
    
- Blockers or stalled tickets
    
- Incidents or dependencies
    
- Team availability (leave, overload)
    

👉 Goal: **spot risk early**, not manage tasks.

---

### 2️⃣ Daily Scrum (15 min)

Your role:

- Ensure the meeting stays focused
    
- Encourage transparency
    
- Surface blockers
    
- Avoid turning it into a status meeting
    

You **don’t run the meeting** — you **facilitate outcomes**.

---

## 🧑‍💻 Mid-Day (Core work)

### 3️⃣ Remove impediments (ongoing)

This is the real job.

Examples:

- Unclear requirements → align PM & team
    
- Environment issues → escalate to platform / DevOps
    
- Cross-team dependency → coordinate timelines
    
- Stakeholder interruptions → shield the team
    

This work is **continuous**, not scheduled.

---

### 4️⃣ Coach individuals & teams

You might:

- Coach a PM on backlog clarity
    
- Help an engineer raise a risk
    
- Support a new team member
    
- Address conflict or misalignment
    

Often happens in **1:1s or small group chats**.

---

### 5️⃣ Align with Product & Engineering leads

Daily or frequent syncs to:

- Re-prioritize work if needed
    
- Adjust scope to reality
    
- Make trade-offs visible
    
- Prepare for upcoming risks
    

This is where your **product background helps a lot**.

---

## 📆 Sprint / Weekly activities

### 6️⃣ Backlog refinement (1–2× per week)

You help ensure:

- Stories are ready
    
- Dependencies are visible
    
- Scope matches capacity
    
- Technical debt is acknowledged
    

You don’t write stories — you **enable readiness**.

---

### 7️⃣ Sprint review

You:

- Ensure stakeholders see real outcomes
    
- Encourage feedback
    
- Protect the team from scope creep mid-sprint
    

---

### 8️⃣ Retrospective

This is **critical**.

You:

- Facilitate honest reflection
    
- Turn insights into actions
    
- Track whether changes actually stick
    

Good Scrum Masters measure **improvement over time**, not just discussion.

---

## 📊 Ongoing responsibilities

### 9️⃣ Improve delivery flow

You look at:

- Cycle time
    
- Throughput
    
- Bottlenecks
    
- WIP limits
    

Then:

- Suggest experiments
    
- Coach the team
    
- Review results
    

---

### 🔟 Stakeholder communication

You help translate:

- Delivery reality → business expectations
    
- Business pressure → team priorities
    

Without blaming either side.


[Core Tech Stack] Programming & Frameworks: TypeScript, Next.js (frontend), Nest.js (backend) Infrastructure & Cloud: AWS (EC2, RDS, S3, IAM, etc.), VPS Providers, Cloud service orchestration Database & ORM: PostgreSQL (on AWS RDS), Prisma Containerization & Deployment: Docker, Kubernetes (orchestration, scaling, monitoring) Expect both theoretical and practical questions, such as how you structured projects, solved real-world issues, and optimized system performance using this stack. [Technical Domains] Web2: Classic client-server architectures, REST APIs, CI/CD, DevOps best practices AI: Experience with ML/AI integrations, model deployment, data pipelines, AI-assisted features Web3: Smart contract development, blockchain integration, wallet systems, decentralized app (dApp) architectures You may be asked about cross-domain overlaps (e.g., AI-powered features within a Web2 and Web3 system, or integrating Web3 identity with Web2 applications, etc.). [Hands-on Experiences] Managing Development Pipelines (SDLC) Experience designing, implementing, and maintaining end-to-end pipelines Tools you’ve used (e.g., GitHub Actions, Jenkins, GitLab CI/CD) How you’ve handled testing, code quality, release cycles, and rollback strategies Code Organization & Separation of Concerns How you structured applications by separating business logic from I/O logic (e.g., controllers, services, repositories) Use of design principles such as SOLID, Clean Architecture, or Domain-Driven Design (DDD) Examples of how this approach improved maintainability, testing, or scaling System Architecture & Design Designing scalable, maintainable systems (microservices, monoliths, or hybrid) Familiarity with design patterns (e.g., Factory, Singleton, Observer, CQRS, Event Sourcing) Trade-offs you’ve considered when choosing architectures or patterns Real examples of how your designs improved performance, resilience, or cost efficiency Overall, you should be ready to discuss both technical depth (hands-on implementation) and strategic thinking (design choices, trade-offs, scaling, maintainability).

web2 + web 3 integration

Frontend (Next.js)  
↓  
Backend (NestJS)  
↓  
Blockchain node  
↓  
AI service  
↓  
Database



User connects wallet  
↓  
Frontend asks user to sign message  
↓  
Backend verifies signature  
↓  
Backend creates session / JWT



Modern platforms often combine Web2 infrastructure with Web3 blockchain integrations and AI-powered features. Web2 services handle APIs, data storage, and business logic. Web3 components manage decentralized assets or identities through smart contracts. AI services can analyze user behavior, automate workflows, or enhance decision-making. The backend usually acts as the bridge connecting these systems.”